...
import { downloadFileOrFolder } from "../file-download";
import { FilePreviewModal } from "./file-preview-modal";

// ----------------------------------------------------------------------

export function FileManagerFileItem({ file, selected, onSelect, onDelete, sx, ...other }) {
  const [favorite, setFavorite] = useState(file.isFavorited);
  ...
  const handleDownload = async () => {
    await downloadFileOrFolder(file);
  };
    ...
    >
      {file.type !== "folder" && (
        <IconButton
          ...
          sx={{
            mr: 0.5,
            color: isPreviewActive ? "primary.main" : "default",
  ...
  const renderFileDetailsDrawer = () => (
    <FileManagerFileDetails
      file={file}
      favorited={favorite}
      onFavorite={(event) => setFavorite(event.target.checked)}
      onCopyLink={handleCopy}
  ...
  return (
    <>
      ...
      <FilePreviewModal open={isPreviewOpen} onClose={handlePreviewClose} file={file} />
    </>
  );
}
