...
export const _appRelated = [
  "Mevzuatlar",
  "Genelgeler",
  "Mali İşler Aralık Raporu",
  "Bilgi İşlem Personel Listesi",
  "Doğrudan Alım Mevzuatı",
].map((name, index) => ({
  id: _mock.id(index),
  name,
  ...
  price: [5].includes(index) ? _mock.number.price(index) : 0,
}));
...
export const _appInvoices = Array.from({ length: 5 }, (_, index) => {
  const category = [
    "Klima arızasının giderilmesi.",
    "Harici monitör alımı.",
    "103 numaralı odanın temizlenmesi..",
    "Servis güzergahına durak eklenmesi.",
    "Toplantı salonunun düzenlenmesi.",
  ][index];

  const status = ["Tamamlandı", "İptal Edildi", "İşleme Alındı", "Tamamlandı", "Tamamlandı"][index];

  return {
    id: _mock.id(index),
    invoiceNumber: `TLP-19${index}`,
    category,
    status,
  };
});
...
export const _appFeatured = Array.from({ length: 3 }, (_, index) => ({
  ...
  coverUrl: _mock.image.haber(index),
}));
